...
#### Don't worry about the amount of exercises you're "supposed to do"

How far we manage to crunch through all of the exercises will depend on
everybody's level of confidence in programming, data science _and_ population
genetics. The contents of the activities as a whole are designed so that no matter
the point that we run out of time, you will take home enough knowledge of
...
which is effectively the same thing as running this:
...
With that out of the way, **simulate a tree sequence which is 50 Mb long, with
a recombination rate 1e-8, and overlay mutations on it at a rate 1e-8.
Check that it has mutations either by typing out `ts` in the console and looking
for a "Mutations" section of the summary, or by using the function `ts_table(ts, "mutations")`.
Then count how many individuals you have recorded for each population using the
table produced by `ts_samples()`.**
...
# We can see in the summary that now there are mutations present
...
## Part 3: Tree-sequence simplification and EIGENSTRAT conversion
...
## Part 4: Converting a tree sequence into EIGENSTRAT
...
Which creates three files `.ind`, `.snp`, and `.geno` as `path/to/a/desired/EIGENSTRAT/prefix.ind`,
`path/to/a/desired/EIGENSTRAT/prefix.snp`, and `path/to/a/desired/EIGENSTRAT/prefix.geno`,
just as you would expect for any EIGENSTRAT file.

**Take your simplified tree sequence `ts_BC0` from above, and convert it to
EIGENSTRAT format under the prefix `data/BC0`.**

::: callout-note
### Click to see the solution
```{r}
# Let's repeat the entire process just so that it's all perfectly clear
samples <- ts_samples(ts)
...
## Part 5: Inspect the EIGENSTRAT data produced by _slendr_
...
## Part 6: Principal Component Analysis on the simulated data
...
**Plot the PCA from your subset of present-day individuals from "popB" and "popC",
now converted to `"data/BC0"` EIGENSTRAT prefix from the `ts_BC0` tree sequence.
Color each individual by their population label, and visualize PC 1 vs PC 2.**
