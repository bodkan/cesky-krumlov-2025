...
As you're working on the following exercises, some reference materials might come
in handy. In general, pretty much all of the exercises are based around my
popgen simulation R package [_slendr_](https://slendr.net). Whenever you need
help, I recommed consulting the following reference materials:

1.  First and foremost, you should refer to the slides with the _slendr_ "crash
course" which we startedour activity session with. Ideally, they should cover pretty
...
(the latter being probably a bit more practical for reference rather than
searching for help through individual slides). If these Quarto Pub links
are not working, it could be because their servers are overloaded. In this case,
you can simply download `slides.html` or `handouts.html` from the
[GitHub repository](https://github.com/bodkan/cesky-krumlov-2025/) for this activity.

2.  Relevant tutorials on the [_slendr_ website](https://www.slendr.net) which
...
3.  [Manual pages](https://www.slendr.net/reference/index.html) of all available
_slendr_ functions. Note that you can get the help page of every _slendr_ R
`function()` by typing `?function` in the R console. For instance, typing
...
the exercise materials (slides, code, data, everything) to your working machine
...
instance) and navigate to your project directory via `File -> Open Project...`
in the RStudio menu.
...
It's unclear whether we will manage to go through the entirety of the selection
-based exercise. However, to be able to do this, having SLiM at least version 4.2
(and it being available in your unix `$PATH!`) is required. If this isn't possible
for your, don't worry. You'll be able to do most of that exercise even without
SLiM by using cached results of the SLiM simulation that I provided.
...
Your goal for each exercise is to write a complete R script script. I suggest
you name the script for each exercise as `exercise1.R`, `exercise2.R`, etc.,
just to keep things tidy and easy to troubleshoot if needed.

Unless you have a strong preference for another editor or IDE, I recommend you
use RStudio (either on your machine or in the cloud, as provided by the
workshop organizers, depending on where you did the setup steps above). If you
like Emacs, vim, VSCode, or anything else, really, feel free to use that as
long as you are able to plot and inspect figures.

Each exercise is compose of individual _parts_, which are designed to build
one upon the other in the order they are specified.

#### Don't worry about the number of exercises you're _"supposed to do"_

How far we manage to crunch through in this session will largely depend on
...
the point we run out of time, you will take home enough knowledge of
_slendr_ to be able to immediately apply it to your own projects.

**After each exercise I will pause a little bit, go through the intended
solutions and answer questions.** If you're way ahead, there are bonus exercises
(which I will not be going through with everyone) -- those should provide at
least some entertainment for you while the rest of us are catching up. If you're
_really_ far ahead, just continue to following exercises without waiting for the
rest of us.
...
hidden) under each exercise section. Always try to work on a solution on your
...
you can implement everything yourself from scratch does not actually matter at all.**
...
which we discussed in the ["crash course"](https://bodkan.quarto.pub) at the
...
To visualize a _slendr_ model, you can use the function `plot_model()`.
**Plot your compiled `model` to make sure you programmed it correctly!**

::: aside
**Note:** Plotting of models can be sometimes a little wonky. When plotting your
...
`model` will always mean a variable containing a compiled _slendr_ model object,
...
so we'll be using that. However, from a popgen theoretical perspective, both
will give you the same results for any given compiled _slendr_ model (up to
some level of stochastic noise, of course).

Here's how you can use the function to simulate a tree sequence:
...
**Hint:** The function has also arguments `debug` and `run` which can be extremely
useful for debugging.

**Simulate a tree sequence from your compiled `model` using the `msprime()`
engine, storing it to a variable `ts` as shown right above. Then experiment with
setting `debug = TRUE` (this prints out _msprime_'s own debugging summary) and
...
As we'll see later, _slendr_ tries to provide an R-friendly interface to
accessing a subset of _tskit_'s functionality for working with tree sequence and
for computing various popgen statistics.
...
**Use the `ts_table` function to inspect the low-level table-based representation of a tree sequence.** For instance, you can get the table of nodes with `ts_table(ts, "nodes")`, edges with `ts_table(ts, "edges")`, and do the same thing for "individuals", "mutations", and "sites". **Does your tree sequence contain any mutations? If not, why, and how can we even do any popgen with data without any mutations?**
...
There are also two _slendr_-specific functions called `ts_samples()` and `ts_nodes()`. **You can run them simply as `ts_samples(ts)` and `ts_nodes(ts)`. What do the tables they produce represent?**
...
do this by providing the combined sampling schedule as the `samples =` argument
of the function `msprime` you used above.** Don't worry about overwriting the
previous contents of `ts <- msprime(...)` in your script, just replace your old
`msprime()` call with the new one which uses the `schedule` for your customized
sampling.
...
**Inspect the tree-sequence object saved in the `ts` variable agains. You can
also do a similar thing via the table produced by the `ts_samples()` function.
You should see a much smaller number of individuals being recorded.**
...
Do you have a favourite model organism or you study another species? **Take some of the most important features of its demographic history and program them as a new _slendr_ model.** If you'd like, you will be able to work with it even in later exercises.
:::
...
As a sanity check, make sure the tree sequence does contain a set of sample
...
The model plotted above makes a fairly clear prediction on what would be the
...
Do you get results you would expect from the model?**
...
by the `ts_samples()` function, _slendr_ provides a useful helper `ts_names()`
which only returns the names of individuals.
...
gives you the heterozygosity for each individual.**
...
**Hint:** Again, you can use the same concept of `sample_sets =` we discussed in the previous part. In this case, the function computes _pairwise_ divergence between
each element of the list given as `sample_sets =` (i.e., for each vector of
individual names).
...
Roughly speaking, we can understand this computation in terms of comparing the
...
My primary motivation for designing _slendr_ was to make demographic modelling
in R as trivially easy and fast as possible, focusing exclusively on neutral
models. However, after _slendr_ became popular, people have been asking me
about the possibility of simulating natural selection. After all, a large
part of _slendr_'s functionality deals with population genetic models across
geographical landscapes, which requires SLiM. Even though these models are
...
When you visualize the model, you might recognize it as a very simplified model
...
sampling `schedule` defined above.**
...
**Do you see any striking differences in the Tajima's D values across populations? Check [this](https://en.wikipedia.org/wiki/Tajima%27s_D#Interpreting_Tajima's_D)
...
Tajima's D value significant enough for the entire chromosome. Fortunately,
thanks to the flexibility of the _tskit_ module, the _slendr_ function 
`ts_tajima()` has an argument `windows =`, which allows us to specify the
coordinates of windows into which a sequence should be broken into, with Tajima's
D computed separately for each window. Perhaps this will allow us to see the
impact of positive selection?

**Define a variable `windows` which will contain a vector of coordinates of
100 windows, starting at position `0`, and ending at position `10e6` (i.e. end
of our chromosome). Then provide this variable as the `windows =` argument of
`ts_tajima()`. Save the result of `ts_tajima()` into the variable `tajima_wins`,
...
etc., by following the instructions above.**
...
We can do this by specifying calling the `slim()` function as `path <- slim(..., path = TRUE)` (so with the extra `path =` argument). This will return a path
to where the `slim()` engine saved all output files.

**Run a simulation from the modified model of selection with the `slim()` engine
as instructed in 1. and 2. above, then use the `list.files(path)` function in R
to take a look in the directory. Which files were produced by the simulation?**
...
visually. The allele was programmed to occur in EURopeans 15 thousand years ago,
and was configured to have been under very strong selection of $s = 0.15$.
Do the trajectories make sense given the demographic model of European prehistory
plotted above?**
...
To read a tree sequence simulated with `slim()` in the customized setup, we need
to do a bit of work. To simplify things a bit, here's the R code which makes it
possible to do. Just copy it in your `exercise4.R` script as it is:
...
in learning more. For the time being, it suffices to say that we can plug the
...
That's all folks! I hope that this session of practical exercises has convinced
you that _slendr_ is a very useful tool to get from an idea about a model or
a statistic to a concrete tangible result in just a few lines of R code, and
that it presents a way to build intuition and test hypotheses very quickly,
without having to leave the convenience of the R environment.
