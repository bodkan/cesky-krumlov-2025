...
which a "CHIMP" and "hominin_ancestor" populations would split, etc.) but
generally this is too annoying to do and normally it's unnecessary. Feel free
to write the model so that "CHIMP" is the first population, then "AFR" population
splits from it, etc... Although it probably isn't the most accurate way to
describe the real evolutionary history, it simplified the coding significantly.
