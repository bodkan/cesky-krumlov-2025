...
sampling `schedule` defined above. Use the function `ts_mutate()` to overlay
mutations on the tree sequence, with a mutation rate `1e-8`.**
...
that we have customized using `substituta_values()` above). We simply do this by
switching from this:
